...
const SalesChart: React.FC<SalesChartProps> = () => {
  ...
  const allYears = useMemo(() => {
    ...
    sellOrders.forEach(order => years.add(getYear(parseISO(order.orderDate)))); // Parse ISO string
    ...
    if (displayMode === 'single') {
      if (chartType === 'yearly') {
        ...
        monthsInYear.forEach(month => {
          monthlySales[format(month, 'MMM')] = 0;
        });

        filteredOrders.forEach(order => {
          const orderDate = parseISO(order.orderDate); // Parse ISO string
        ...
        return monthsInYear.map(month => ({
          name: t(format(month, 'MMM').toLowerCase() as keyof typeof t),
