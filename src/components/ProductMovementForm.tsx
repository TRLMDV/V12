...
const ProductMovementForm: React.FC<ProductMovementFormProps> = ({ movementId, onSuccess }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (sourceWarehouseId === '' || destWarehouseId === '') {
      showAlertModal(t('validationError'), t('pleaseSelectBothSourceAndDestinationWarehouses'));
      return;
    }
    if (sourceWarehouseId === destWarehouseId) {
      showAlertModal(t('validationError'), t('sourceAndDestinationWarehousesCannotBeSame'));
      return;
    }

    const newItems = movementItems.filter(item => item.productId !== '' && item.quantity > 0);
    if (newItems.length === 0) {
      showAlertModal(t('validationError'), t('pleaseEnsureAllItemsHaveASelectedProductAndAQuantityGreaterThanZero'));
      return;
    }
    ...
    for (const item of newItems) {
      ...
      if (stockInSource < item.quantity) {
        ...
        const safeProductName = originalProduct?.name || t('unknownProduct');
        showAlertModal(t('stockError'), `${t('notEnoughStock')} ${safeProductName}. ${t('available')}: ${stockInSource}, ${t('requested')}: ${item.quantity}.`);
        return;
      }
