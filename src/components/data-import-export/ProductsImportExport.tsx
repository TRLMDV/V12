...
const ProductsImportExport: React.FC<ProductsImportExportProps> = ({
  ...
  const handleImportProducts = (data: any[]) => {
    const newProducts: Product[] = data.map((row: any) => ({
      ...
      sku: String(row['SKU'] || ''),
      barcode: String(row['Barcode'] || '') || undefined, // New: Import barcode
    ...
    <div className="mb-6 p-4 bg-white dark:bg-slate-800 rounded-lg shadow-md">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <ExcelImportButton
          ...
          requiredColumns={['Product Name', 'SKU', 'Barcode', 'Description', 'Min. Stock', 'Avg. Landed Cost', 'Image URL']} // New: Add 'Barcode'
        />
        <ExcelExportButton
          ...
          columns={[
            ...
            { header: 'Barcode', accessor: 'barcode' }, // New: Export barcode
